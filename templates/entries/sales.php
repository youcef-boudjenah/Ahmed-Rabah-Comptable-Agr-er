<form method="post" action="/entries/sales" class="max-w-2xl card">
    ...
    <div class="grid grid-cols-2 gap-4">
        ...
        <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">Mois (G50)</label>
            <input type="number" name="period_month" min="1" max="12" value="5" class="input">
        </div>
    </div>
    <div class="grid grid-cols-3 gap-4">
        <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">CA biens</label>
            <input type="text" name="ca_biens" value="0" class="input font-mono">
        </div>
        <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">CA services</label>
            <input type="text" name="ca_services" value="0" class="input font-mono">
        </div>
        <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">CA auto-entrepreneur</label>
            <input type="text" name="ca_auto_entrepreneur" value="0" class="input font-mono">
        </div>
    </div>
    <div>
        <label class="block text-sm font-medium text-slate-700 mb-1">Base IRG acompte G50 (DA)</label>
        <input type="text" name="irg_acompte_base" value="" placeholder="702522" class="input font-mono">
        <p class="text-xs text-slate-400 mt-1">Ex. SEKRANE BOUZIANE → 702 522 × 30% = 210 756,60 DA</p>
    </div>
    <button type="submit" class="btn btn-primary btn-block btn-lg">Enregistrer & calculer G12 / G50</button>
    </div>
</form>
