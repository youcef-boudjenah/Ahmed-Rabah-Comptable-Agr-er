...
<div class="client-picker<?= $compact ? ' client-picker-compact' : '' ?>" data-picker-id="<?= $inputId ?>">
    ...
    <?php if (!$compact): ?>
    <label class="block text-xs font-medium text-slate-600 mb-1.5">Client<?= $required ? ' *' : '' ?></label>
    <?php endif; ?>
    <div class="relative">
        <input type="text" id="<?= $inputId ?>-search" autocomplete="off" placeholder="Rechercher par nom, wilaya, n° cotisant…"
    ...
    <p id="<?= $inputId ?>-hint" class="text-xs text-slate-400 mt-1<?= $compact ? ' hidden' : '' ?>">
        <?= $selectedClient ? htmlspecialchars($selectedClient['secteur'] . ($selectedClient['wilaya'] ? ' · ' . $selectedClient['wilaya'] : '')) : 'Tapez au moins 2 caractères — adapté à 1000+ clients' ?>
    </p>
</div>
<script>
(function() {
    ...
    search.addEventListener('input', () => {
        ...
        timer = setTimeout(async () => {
            ...
            if (!data.results?.length) {
                results.innerHTML = '<p class="p-3 text-xs text-slate-400">Aucun client trouvé</p>';
            } else {
